...
define('ABC_NO_LOG', -1);
function _die($str) {
    SX::syslog("Выполнение скрипта остановлено в классе XLS по причине: $str", '0', $_SESSION['benutzer_id']);
    exit;
}

class DebugOut {

    public $priorities = array(ABC_CRITICAL => 'Критический', ABC_ERROR => 'Ошибка', ABC_ALERT => 'Предупреждение', ABC_WARNING => 'Внимание', ABC_NOTICE => 'Уведомление', ABC_INFO => 'Информация', ABC_DEBUG => 'Отладка', ABC_TRACE => 'Трассировка', ABC_VAR_DUMP => 'Дамп');
    public $_ready = false;
    public $_currentPriority = ABC_DEBUG;
    public $_consumers = array();
    public $_filename;
    public $_fp;
    public $_logger_name;
...
class ExcelFont {
    ...
    function toString(&$record, $index) {
        $retval = sprintf("Индекс Шрифта = %d \nРазмер шрифта =%d\nКурсив = %s\nЗачеркнутый=%s\nPalete=%s\nЖирность = %s Полужирный=%s\n Script = %d\n Подчеркивание = %d\n Имя шрифта=%s<hr>", $index, $record['size'], $record['italic'] == true ? "true" : "false", $record['strikeout'] == true ? "true" : "false", $record['palete'], $record['boldness'], $record['bold'] == true ? "true" : "false", $record['script'], $record['underline'], $record['name']
        );
        return $retval;
    }
...
class DataProvider {
        ...
        switch ($this->_type) {
            case DP_FILE_SOURCE: {
                    if (fseek($this->_data, $this->_baseOfs + $offset, SEEK_SET) == -1)
                        _die("Не удалось найти позицию в файле указанную в смещении.");
    ...
    function ReadFromFat($chain, $gran = 0x200) {
        $rc = '';
        for ($i = 0, $count = count($chain); $i < $count; $i++)
...
class ExcelFileParser {
                ...
                case 0x0500:
                    if (ord($ws[0x0a]) + 256 * ord($ws[0x0b]) < 1994) {
                        ...
                        switch (ord($ws[8]) + 256 * ord($ws[9])) {
                            ...
                            case 3218:
                            case 3321:
                                $this->dbglog->debug("Анализатор BIFF версия - 5");
                                $biff_ver = 5;
                                break;
                            default:
                    ...
                    break;
                case 0x0600:
                    $this->dbglog->debug("Анализатор BIFF версия 8");
                    $biff_ver = 8;
                    break;
                default:
                    return 8;
    ...
    function parse_workbook($f_header, $dp) {
        ...
        $this->dbglog->trace("Анализ заголовка");
        $this->fat = array();
        ...
        if ($small_block != 0xfeffffff) {
            ...
            for ($i = 0, $counts = count($schain); $i < $counts; $i++) {
    ...
    function InitParser() {
        ...
        for ($i = 0, $count = count($header_sig); $i < $count; $i++)
...
class XLSReader {
    ...
    function __construct($fp, $fn) {
        ...
        $this->_exc = new ExcelFileParser();
        $res = $this->_exc->ParseFromString($fc);
        $err = '';
        switch ($res) {
            case 0:
                break;
            case 1:
                $err = 'Ошибка в классе XLS: Невозможно открыть файл.';
                SX::syslog($err, '0', $_SESSION['benutzer_id']);
                return 0;
            case 2:
                $err = 'Ошибка в классе XLS: Файл, слишком маленький чтобы быть файлом Excel';
                SX::syslog($err, '0', $_SESSION['benutzer_id']);
                return 0;
            case 3:
                $err = 'Ошибка в классе XLS: Ошибка чтения заголовка файла';
                SX::syslog($err, '0', $_SESSION['benutzer_id']);
                return 0;
            case 4:
                $err = 'Ошибка в классе XLS: Ошибка чтения файла';
                SX::syslog($err, '0', $_SESSION['benutzer_id']);
                return 0;
            case 5:
                $err = 'Ошибка в классе XLS: Это - не файл Excel или файл, сохраненный в Excel < 5.0';
                SX::syslog($err, '0', $_SESSION['benutzer_id']);
                return 0;
            case 6:
                $err = 'Ошибка в классе XLS: Файл поврежден';
                SX::syslog($err, '0', $_SESSION['benutzer_id']);
                return 0;
            case 7:
                $err = 'Ошибка в классе XLS: Ненайдены данные в Excel  файле';
                SX::syslog($err, '0', $_SESSION['benutzer_id']);
                return 0;
            case 8:
                $err = 'Ошибка в классе XLS: Неподдерживаемая версия файла';
                SX::syslog($err, '0', $_SESSION['benutzer_id']);
                return 0;
            default:
                $err = 'Ошибка в классе XLS: Неизвестная ошибка';
                SX::syslog($err, '0', $_SESSION['benutzer_id']);
                return 0;
        }

        $ws_number = count($this->_exc->worksheet['name']);
        if ($ws_number < 1) {
            SX::syslog('Ошибка в классе XLS: Не найден рабочий лист в Excel файле', '0', $_SESSION['benutzer_id']);
            return 0;
        }

        $ws_n = 0;
        $ws = $this->_exc->worksheet['data'][$ws_n];
        $this->_rows = $this->_parse($ws);
    ...
    function _get_xl_item($exc, $data) {
        switch ($data['type']) {
            case 0:
                $ind = $data['data'];

                if ($exc->sst['unicode'][$ind])
                    return iconv("UTF-16LE", "WINDOWS-1251", $exc->sst['data'][$ind]);
                else
                    return iconv("WINDOWS-1251", "UTF-8", $exc->sst['data'][$ind]);
    ...
    function _parse($ws) {
        ...
        if (!($max_row > 0 && $max_col > 0)) {
            SX::syslog('Ошибка в классе XLS: Пустой рабочий лист', '0', $_SESSION['benutzer_id']);
            return 0;
        } else {
            $data = $ws['cell'];
            foreach ($data as $i => $row) {
                for ($j = 0; $j <= $ws['max_col']; $j++) {
                    $col_p++;
                    @$rows[$row_p][$col_p] .= @ $this->_get_xl_item($this->_exc, $row[$j]);
                }
                $col_p = 0;
                $rows[++$row_p] = array();
            }
