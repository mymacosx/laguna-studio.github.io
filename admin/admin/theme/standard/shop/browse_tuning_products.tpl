...
function setElem(s,i) {
    parent.document.getElementById('tun').innerHTML = parent.document.getElementById('tun').innerHTML + '<input type="checkbox" name="tuningteile[]" value="' + i + '" checked="checked">' + s + '<br />';
}
//-->
</script>
        ...
        <td>
          {strip}
            <select style="width: 385px;" onchange="eval(this.options[this.selectedIndex].value);">
              ...
              {foreach from=$categs_ass item=categs}
                <option style="{if $categs->Bold == 1}font-weight: bold;{/if}" {if isset($smarty.request.c) && $smarty.request.c == $categs->catid}selected="selected"{/if} value="location.href = 'index.php?noout=1&noframes=1&do=shop&sub=get_prods_tuning&c={$categs->catid}';">{$categs->Expander}{$categs->Name}</option>
              {/foreach}
            </select>
          {/strip}
        </td>
