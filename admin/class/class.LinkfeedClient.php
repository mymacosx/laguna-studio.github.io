...
class LinkfeedClient {
    ...
    function load_links() {
        if ($this->lc_multi_site) {
            $this->lc_links_db_file = SX_DIR . '/temp/links/linkfeed.' . $this->lc_host . '.links.db';
        } else {
            $this->lc_links_db_file = SX_DIR . '/temp/links/linkfeed.links.db';
        }
        ...
        if ($this->lc_ignore_tailslash && $this->lc_request_uri[strlen($this->lc_request_uri) - 1] == '/')
            $this->lc_request_uri = substr($this->lc_request_uri, 0, -1);
