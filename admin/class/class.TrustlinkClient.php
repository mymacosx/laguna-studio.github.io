...
class TrustlinkClient {
    ...
    function load_links() {
        if ($this->tl_multi_site) {
            $this->tl_links_db_file = SX_DIR . '/temp/links/trustlink.' . $this->tl_host . '.links.db';
        } else {
            $this->tl_links_db_file = SX_DIR . '/temp/links/trustlink.links.db';
        }
        ...
        if ($this->tl_cache) {
            //check dir
            if (!is_dir(SX_DIR . '/temp/links/' . $this->tl_cache_dir)) {
                if (!@mkdir(SX_DIR . '/temp/links/' . $this->tl_cache_dir)) {
                    return $this->raise_error("There is no dir " . SX_DIR . '/temp/links/' . $this->tl_cache_dir . ". Fail to create. Set mode to 777 on the folder.");
                }
            }
            //check dir rights
            if (!is_writable(SX_DIR . '/temp/links/' . $this->tl_cache_dir)) {
    ...
    function cache_filename($i) {
        ...
        return SX_DIR . '/temp/links/' . $this->tl_cache_dir . $this->tl_cache_filename . $host . '.db' . $i;
    }
    ...
    function raise_error($e) {
        ...
        SX::syslog('Ошибка в модуле Trustlink! ' . $e, '3', $_SESSION['benutzer_id']);
        return false;
    }
    ...
    function build_links($n = null) {
        ...
        if (isset($this->tl_links['__trustlink_start__']) && strlen($this->tl_links['__trustlink_start__']) != 0 &&
                (in_array(IP_USER, $this->tl_links['__trustlink_robots__']) || $this->tl_force_show_code)
        ) {
            $result .= $this->tl_links['__trustlink_start__'];
        }

        if (isset($this->tl_links['__trustlink_robots__']) && in_array(IP_USER, $this->tl_links['__trustlink_robots__']) || $this->tl_verbose) {
        ...
        $tpl = '<ul><{block}><li><{head_block}><p><{link}></p><{/head_block}><p><{text}></p><p><{host}></p></li><{/block}></ul>';
