...
<div id="zubc_prodmessage" style="display: none">
  <br />
  <p class="h3">{#Shop_ProdAddedToBasket#}</p>
  <p>{#LoginExternActions#}</p>
  <input class="shop_buttons_big" type="button" id="zubc_yes" value="{#Shop_go_basket#}" />
  <input class="shop_buttons_big_second" type="button" id="zubc_no" value="{#WinClose#}" />
  <br />
  <br />
</div>
  <div class="shop_contents_box_tabs">
    {foreach from=$Zub_c_products_array item=p name=tuningparts}
          ...
          <div style="float: right">
            ...
            {else}
              {if empty($p.Vars) && $p.Lagerbestand > 0 && $p.Preis > 0 && empty($p.Frei_1) && empty($p.Frei_2) && empty($p.Frei_3)}
                <form method="post" class="zubc_products" id="zubc_{$p.Id}" action="index.php?p=shop&amp;area={$area}">
                  ...
                  <input type="hidden" name="mylist" id="mylist_zubc_{$p.Id}" value="0" />
                  <input type="hidden" name="ajax" value="1" />
                  <noscript>
                  <input type="hidden" name="ajax" value="0" />
                  </noscript>
                  <button class="shop_buttons_big" type="submit">{#Shop_toBasket#}</button>&nbsp;
                  <button class="shop_buttons_big_second" onclick="document.getElementById('mylist_zubc_{$p.Id}').value = '1';" type="submit">{#Shop_WishList#}</button>
                </form>
              {else}
                <form method="post" action="{$p.ProdLink}{if isset($smarty.request.blanc) && $smarty.request.blanc == 1}&amp;blanc=1{/if}">
