...
<div id="zuba_prodmessage" style="display: none">
  ...
  <input class="shop_buttons_big" type="button" id="zuba_yes" value="{#Shop_go_basket#}" />
  <input class="shop_buttons_big_second" type="button" id="zuba_no" value="{#WinClose#}" />
  <br />
  <br />
</div>
  <div class="shop_contents_box_tabs">
    {foreach from=$Zub_a_products_array item=p name=accessories}
          ...
          <div style="float: right">
            ...
            {else}
              {if empty($p.Vars) && $p.Lagerbestand > 0 && $p.Preis > 0 && empty($p.Frei_1) && empty($p.Frei_2) && empty($p.Frei_3)}
                <form method="post" class="zuba_products" id="zuba_{$p.Id}" action="index.php?p=shop&amp;area={$area}">
                  ...
                  <button class="shop_buttons_big_second" onclick="document.getElementById('mylist_zuba_{$p.Id}').value = '1';" type="submit">{#Shop_WishList#}</button>
                </form>
              {else}
                <form method="post" action="{$p.ProdLink}{if isset($smarty.request.blanc) && $smarty.request.blanc == 1}&amp;blanc=1{/if}">
